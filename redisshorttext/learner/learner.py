...
def read_SVMProblem(src,redishost,redisport):
    status = c_int64()
    svmprob = util.read_problem(src.encode(), 0, pointer(status),redishost,redisport)  # bias = 0 is required
    ...
    if status == -3:
        raise IOError("cant connect redis")
...
class LearnerProblem(liblinear.problem):
    def __init__(self, src,redishost,redisport):
        #svmprob = util.read_problem(src.encode(), 0)  # bias = 0 is required
        svmprob = read_SVMProblem(src,redishost,redisport)  # bias = 0 is required
...
def train(data_file_name, learner_opts="", liblinear_opts="",redishost="localhost",redisport=6379):
    """
    ...
    *data_file_name* is the file path of the LIBSVM-format data. *learner_opts* is a
    ...
    learner_prob = LearnerProblem(data_file_name,redishost,redisport)
